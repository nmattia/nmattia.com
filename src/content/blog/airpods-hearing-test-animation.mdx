...
Since the shader program runs once for every pixel to determine the pixel's output (opacity here), I find it helpful to imagine the canvas from the perspective of a single pixel -- hang with me for a second. So if we are a pixel, and we know our position, how do we go about figuring out our opacity so that a dot is displayed as a result? As mentioned above, only the pixel's position (which we'll denote with `uv` here) is input to the program.

> [!NOTE]
>
> My friend Bas read a preview of this article and pointed me to Conal Elliot's [Functional Images](http://conal.net/papers/functional-images/). In a sense, fragment shaders are a way of describing images functionally, where you give a mathematical expression for what each pixel should be, and let the GPU do the rest!
...
This is the name of the game when doing ~~shady~~ _shader_ stuff: find a way to describe a shape mathematically around the origin; to then move it, "shift the canvas" by subtracting the actual center before performing the check. If you ever need to draw shapes more complex than dots (and I hope you do!), see Inigo Quilez’s 2D [distance functions](https://iquilezles.org/articles/distfunctions2d/) which describe many more shapes like rounded boxes, triangles, stars, etc.
...
Let's assume we have three sheets of the same material which is slightly opaque, and let's say each sheet lets 20% of the light through. We'll say each sheet is 80% opaque -- this might not be scientifically precise, but it serves as a helpful mental model. Now let's picture a photon (light particle) that has to go through all three sheets of the material.
