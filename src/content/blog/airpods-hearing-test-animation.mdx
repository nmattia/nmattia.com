...
Since the shader program runs once for every pixel to determine the pixel's output (opacity here), I find it helpful to try and see the canvas (made of all the pixels) from a pixel's perspective (yeah hang with me for a minute). So if we are a pixel, and we know our position, how do we go about figuring out our opacity so that a dot is displayed as a result? As mentioned above, all only the pixel's position (which we'll denote with `uv` here) is input to the program.
...
In order to simplify things we will work with [polar coordinates](https://en.wikipedia.org/wiki/Polar_coordinate_system). Quick refresher, this coordinate system lets us express the position $(x, y)$ in terms of $rho$ (`rho`) and $theta$ (`theta`), where $rho$ is the distance to the origin and $theta$ the "polar angle".
...
Handling user interactions like clicks (as in the very first animation) is left as an exercise to the reader (see [here](TODO) for the first animation's source). And while you're at it, why not play with the colors a bit too!?
