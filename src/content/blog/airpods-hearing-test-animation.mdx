...
If you squint at the animation, you'll see it's actually made up of lots of small dots (3328 of them to be exact). We'll use a simplified version to explain the various things necessary to draw it: first draw a disc, then radial symmetries, and finally overlapping discs with alpha.
...
Let's start simple: let's draw a disc. If we're inside the disc, the pixel is opaque. Otherwise, it is transparent.
...
This gives us the disc of radius `1.` shown below.
...
_Two of the many pixels rendering a disc of radius `r` centered at `c`_
...
So effectively we run the same calculation as above to check whether we are inside a unit circle, but we first scale ourselves down (or scale the canvas up, depending on how you look at it). Blah blah step(radius, length(uv)).
...
_Dots described for a single slice and separated by distance `d`_
