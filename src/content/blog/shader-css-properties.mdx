...
> [!NOTE]
>
> The `vPosition` is a [varying](https://thebookofshaders.com/glossary/?search=varying) variable, though what exactly this means is _not important_ here. What is somewhat relevant to know is that it is set up (somewhere else) to range between -1 and 1 (or similarly between - and + the aspect ratio).
>
...
The implementation of `parseRGBA` is pretty straightforward, though note that the browser may return the string as either `rgb(r,g,b)` or `rgba(r,g,b,a)` (even though modern CSS syntax allows skipping the commas, the value returned from the browser has commas between the `r`, `g`, `b` and `a` values):
