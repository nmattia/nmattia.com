...
The colors used in the (fragment) shader (used to draw the animation) are read from the page's styles. This allows fancy WebGL animations to blend in smoothly in websites, be derived from design systems, and react to changes (like dark mode).
...
This is done by removing the local `color` variable and instead using a new variable (well, identifier) `uColor`, marked as `uniform`:
...
In this case we see that looking up the value of a property returns the value _verbatim_, as it was set:
...
This means we would have to parse every possible way the color may be specified, of which there is [a ton](https://developer.mozilla.org/en-US/docs/Web/CSS/color_value#syntax).

Instead, we can rely on the fact that once the color is attached to an element, and if that element is attached to the DOM, the browser will return the value in easy-to-parse RGB format. Let's set the body's `color` to `--col-1`:
...
Hopefully this gave you a good overview of the technique, which is also implemented in [`quad-shader`](https://github.com/nmattia/quad-shader/blob/6d1ba7b3b70801b0d03bfdf5b04f8984eb743be6/index.ts#L14-L38). Enjoy and don't hesitate to reach out for questions & feedback!
